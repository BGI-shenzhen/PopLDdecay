...
int GetBestBase (const string & Genotype, const map <string ,char > & SNP_Allele , const map <string,string > & SNP_back_Allele ,  vector <BaseType> &  Base_list)
{
	vector<string>  Base1 ;
	...
	{
		map<string,string>::const_iterator it_ba = SNP_back_Allele.find(Base1[ii]);
		if (it_ba == SNP_back_Allele.end()) continue;
		const string & A_tmp = it_ba->second;
		Count[A_tmp[0]]++;
		Count[A_tmp[1]]++;
	}
	char best_base='N';
	...
	map<string,char>::const_iterator it_sa = SNP_Allele.find(Het_base);
	string C_base = (it_sa != SNP_Allele.end() ? string(1, it_sa->second) : "N") + tmp;
...
int Read_SubPopVCF_IN(In3str1v * paraFA04, Para_18 * para_18 , map <string,map <llong, vector <BaseType> > > &  SNPList ,int & Flag_for_pro )
{
	...
	std::vector <const char*> spline;
	spline.reserve(Asample);
	for (int ii=0 ; ii<Asample ; ii++)
	{
		spline.push_back("tmpstr");
	}
	...
	while(!VCFIN.eof())
	{
		...
		split3(line,spline,Vsample,Asample);
		...
		for (int kk=0 ; kk< NumberSubGroup ; kk++)
		{
			...
			else
			{
				...
				Count[spline[SampleSite[kk]][0]]++;
				Count[spline[SampleSite[kk]][2]]++;
			}
		}
...
//			ABase=spline[SampleSite[kk]][0];
			...
			else
			{
				...
				{
					genotype.push_back(spline[SampleSite[kk]][0]);
					genotype.push_back(spline[SampleSite[kk]][2]);
				}
			}
		}
		...
		Site = strtoll(Vsample[1].c_str(), nullptr, 10);
...
int Read_VCF_IN(In3str1v * paraFA04, Para_18 * para_18 , map <string,map <llong, vector <BaseType> > > &  SNPList ,int & Flag_for_pro )
{
	...
	if (paraFA04->TF2)
	{
		...
		while(!VCFIN.eof())
		{
			...
			split3(line,spline,inf,Asample);
			...
			for (int jj=9 ; jj< Asample ;jj++ )
			{
				...
				else
				{
					...
					{
						genotype.push_back(spline[jj][0]);
						genotype.push_back(spline[jj][2]);
					}
				}
			}
...
int Read_Genotype_IN(In3str1v * paraFA04, Para_18 * para_18 , map <string,map <llong, vector <BaseType> > > &  SNPList,int &  Flag_for_pro )
{
	...
	while(!SNP.eof())
	{
		...
		Site = strtoll(inf[1].c_str(), nullptr, 10);
...
int Read_SubPopVCF_IN_Phase(In3str1v * paraFA04, Para_18 * para_18 , map <string,map <llong, vector <BaseType> > > &  SNPList ,int & Flag_for_pro )
{
	...
	while(!VCFIN.eof())
	{
		...
		for (int kk=0 ; kk< NumberSubGroup ; kk++)
		{
			...
			{
//				BBase=Genotype[2];
				genotype.push_back(spline[SampleSite[kk]][0]);	// phase VCF
				genotype.push_back(spline[SampleSite[kk]][2]);  // phase VCF
			}
		}
	...
	if (paraFA04->TF2)
	{
		...
		while(!VCFIN.eof())
		{
			...
			for (int jj=9 ; jj< Asample ;jj++ )
			{
				...
				{
					genotype.push_back(spline[jj][0]);   // phase VCF
					genotype.push_back(spline[jj][2]);   // phase VCF
				}
			}
